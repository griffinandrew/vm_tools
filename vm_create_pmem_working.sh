...
if sudo virsh domstate "$VM_NAME" &>/dev/null; then
    ...
    sudo virt-install \
      ...
      --qemu-commandline="-machine q35,nvdimm=on -m slots=5,maxmem=${MAX_MEM_KIB}k -object memory-backend-file,id=mem1,share=on,mem-path=${PMEM_PATH},size=${PMEM_SIZE_BYTES},align=2M,pmem=on -device nvdimm,id=nv1,memdev=mem1,label-size=128K" \
      --wait -1
fi
